...
combineddata =data.frame(Time_neighbor_two_step_arrow, Time_neighbor_two_step_tapered)
combineddata = stack(combineddata)
results = wilcox.test(values~ind, combineddata, paired=TRUE)
tasknames[2] = "Time_neighbor_two_step_arrow-Time_neighbor_two_step_tapered"
pvalues[2] = results$p.value
pvalues_adj = p.adjust(pvalues, "bonferroni")
paste(tasknames[1] , "," , pvalues_adj[1])
paste(tasknames[2] , "," , pvalues_adj[2])
...
combineddata =data.frame(Time_neighbor_two_step_tapered, Time_neighbor_two_step_circular)
combineddata = stack(combineddata)
results = wilcox.test(values~ind, combineddata, paired=TRUE)
tasknames[2] = "Time_neighbor_two_step_tapered-Time_neighbor_two_step_circular"
pvalues[2] = results$p.value
pvalues_adj = p.adjust(pvalues, "bonferroni")
paste(tasknames[1] , "," , pvalues_adj[1])
paste(tasknames[2] , "," , pvalues_adj[2])
